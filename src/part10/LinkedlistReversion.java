...
public class LinkedlistReversion {
	public static void main(String[] args) {
		...
		linkList.displayAllNodes();
		//因为这里的操作和c++的指针不一样，所以只能自己新定义一个方法setFirstNode来重新定义这个反转了的链表
		Node first = reversion(linkList.first);
		linkList.setFirstNode(first);
		linkList.displayAllNodes();
	}

	protected static Node reversion(Node head) {
		Node pre = null;
		Node next = null;
		while (head != null) {
			next = head.next;
